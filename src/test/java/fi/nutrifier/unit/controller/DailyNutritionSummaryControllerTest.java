...
import fi.nutrifier.controllers.DailyNutritionSummaryController;
...
@WebMvcTest(DailyNutritionSummaryController.class)
@ActiveProfiles("test")
@Import(SecurityConfig.class)
public class DailyNutritionSummaryControllerTest extends ControllerTestInterface<DailyNutritionSummaryService> {

    protected DailyNutritionSummaryControllerTest() {
    ...
    public void testCreate_ReturnCreated() throws Exception {
        when(service.create(any(UUID.class), any(DailyNutritionSummaryCreateRequest.class)))
                .thenReturn(new ResponseEntity<>(TestObjects.dailyNutritionSummary.toResponse(), HttpStatus.CREATED));

        mockMvc.perform(post(baseUrl)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(TestObjects.dailyNutritionSummary.toCreateRequest())))
    ...
    public void testUpdate_ReturnUpdated() throws Exception {
        when(service.update(any(UUID.class), any(UUID.class), any(DailyNutritionSummaryUpdateRequest.class)))
                .thenReturn(new ResponseEntity<>(TestObjects.dailyNutritionSummary.toResponse(), HttpStatus.OK));

        mockMvc.perform(patch(baseUrl + "/{id}", TestObjects.dailyNutritionSummary.getId().toString())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(TestObjects.dailyNutritionSummary.toUpdateRequest()))
